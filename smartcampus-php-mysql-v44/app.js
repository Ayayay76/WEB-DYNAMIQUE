...
const STORAGE_KEY = "smartcampus_demo_data_v44";
...
const defaultData = {
  ...
  modules: [
    { id: 1, code: "MOD-WEB-BDD", nom: "Module Web & Bases de données", semestre: "S4", courseIds: [1, 3] },
    { id: 2, code: "MOD-MATH", nom: "Module Mathématiques appliquées", semestre: "S4", courseIds: [2] }
  ],
  classEnrollments: [
...
const NOTIFICATION_READS_KEY = "smartcampus_notification_reads_v44";
...
const CALENDAR_DAYS = ["Lundi", "Mardi", "Mercredi", "Jeudi", "Vendredi", "Samedi"];
    ...
    <>
      <SimpleCrudPage
        ...
        filterFields={[
          ...
          { key:"semestre", label:"Semestre", options:[...new Set(data.courses.map((course) => course.semestre).filter(Boolean))].sort() }
        ]}
        onDelete={(id) => {
          updateData({
            ...data,
...
const GradesPage = ({ data, updateData, allowedCourses = null, title = "Gestion des notes", subtitle = "" }) => {
        ...
        <div className="table-wrap">
          <table className="data-table">
            ...
            <tbody>
              ...
              {gradeRows.length === 0 && (
                <tr>
                  <td colSpan="8">Aucune note à afficher.</td>
                </tr>
              )}
            </tbody>
          </table>
              ...
              <tbody>
                {selectedComponents.map((component) => (
                  <tr key={component.id}>
                    ...
                    <td>{getComponentScore(component).toFixed(2).replace(/\.00$/, "")}/20</td>
                    <td>
                      <input
                        type="number"
                        min="0"
...
const StudentApp = ({ data, updateData, currentUser, logout, notificationReads, markNotificationsRead }) => {
  ...
  const generalAverage = studentGrades.length ? (studentGrades.reduce((sum, grade) => sum + Number(moyenne(grade)), 0) / studentGrades.length).toFixed(2) : "0.00";
  ...
  return (
    ...
    >
      ...
      {tab === "grades" && <GenericTable title="Mes notes" subtitle="" items={studentGrades.map((g) => ({ ...g, suivi:categoryAverage(g, "Suivi"), ds:categoryAverage(g, "DS"), projet:categoryAverage(g, "Projet"), moyenne:moyenne(g) }))} columns={[["cours","Cours"],["suivi","Suivi"],["ds","DS"],["projet","Projet"],["moyenne","Moyenne"]]}/>}
...
const TeacherApp = ({ data, updateData, currentUser, logout, notificationReads, markNotificationsRead }) => {
  ...
  return (
    ...
    >
      ...
      {tab === "grades" && <GradesPage data={data} updateData={updateData} allowedCourses={teacherCourses} title="Notes de mes cours"/>}
