...
session_name('SMARTCAMPUS_V44_SESSION');
...
function list_data() {
    ...
    $modules = fetch_all("SELECT id, code, name AS nom, semester AS semestre FROM modules ORDER BY id");
...
function save_all($data) {
    ...
    try {
        ...
        foreach (($data['modules'] ?? []) as $module) {
            execute_sql("INSERT INTO modules (id, code, name, semester) VALUES (?, ?, ?, ?)", [
                $module['id'] ?? null,
                $module['code'] ?? '',
                $module['nom'] ?? '',
                $module['semestre'] ?? '',
